...
class Enemy : Entity {
    ...
    override func collidedWith(body: SKPhysicsBody, contact: SKPhysicsContact, damage: Int = 10) {
        ...
        if health <= 0 {
            ...
            if deathEmitter.parent == nil {
                deathEmitter.runAction(SKAction.sequence([
                        SKAction.waitForDuration(3.0),
                        SKAction.removeFromParent()
                        ]))
                mainScene.particleLayerNode.addChild(deathEmitter)
            }
            deathEmitter.hidden = false
